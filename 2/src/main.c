...
void initVector(double *matrix, int M)
{
    ...
    for ( int i = 0; i < M; ++i ) {
        for(int j = 0 ; j < 16 ; j++){
            matrix[i*16+j] = rand()*1.0/INT_MAX;
        }
    }
...
int checkDifference(double *matrix1, double *matrix2, int N)
{
    for ( int i = 0; i < N; ++i ) {
        for(int j = 0 ; j < 16 ; ++j){
        if (matrix1[i*16+j] != matrix2[i*16+j]) {
            fprintf(stderr, "matrices differ at position [%d]: ", i*16+j);
            fprintf(stderr, "matrix1[%d] = %.5f, ",  i*16+j, matrix1[i*16+j]);
            fprintf(stderr, "matrix2[%d] = %.5f\n",  i*16+j, matrix2[i*16+j]);
            return 0;
        }
        }
    }
...
int main(int argc, char* argv[]){
    ...
    do {
        ...
        input_vector = (double *)malloc(M*16*sizeof(double));
        naiveoutput = (double *)malloc(N*16*sizeof(double));
        optoutput = (double *)malloc(N*16*sizeof(double));
